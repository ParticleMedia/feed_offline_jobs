...
##### task conf
TASK_NAME="local_cross_geo_first_cat"
...
# redis
REDIS_PREFIX="local_geo_first_cat"
REDIS_TTL_SEC=86400
...
FROM (
    ...
    WHERE
      ...
      AND cjv.checked = 1
      AND city_id IS NOT NULL
    GROUP BY city_id,first_cat,type_pid
) t
LEFT JOIN zip_cate_cjv ON t.city_id = zip_cate_cjv.city_id AND t.first_cat = zip_cate_cjv.first_cat
WHERE zip_cate_cjv.users >= ${MIN_ZIP_CATE_USER}
  AND t.ctr > zip_cate_cjv.ctr
  AND t.check >= ${MIN_CHECK}
ORDER BY t.city_id,t.first_cat,t.ctr DESC
LIMIT 1000000
    "
